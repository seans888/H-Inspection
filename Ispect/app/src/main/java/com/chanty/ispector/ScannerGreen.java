...
public class ScannerGreen extends AppCompatActivity {
...
//WHEN SCANNER GETS RESULT VALUES ARE SAVED
    @Override
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if(result != null){
            if(result.getContents()==null){
                ...
                DatabaseReference reference = mFirebaseDatabase1.child(RoomName).child(date).child("Scan-in").child(currentDateTimeString).child("Room");;
                DatabaseReference reference1 = mFirebaseDatabase1.child(RoomName).child(date).child("Scan-in").child(currentDateTimeString).child("User");
