...
public class ScannerYellow1 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_scanner_yellow);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if(result != null){
            if(result.getContents()==null){
                ...
                DatabaseReference reference = mFirebaseDatabase1.child(RoomName).child(date).child("Sign-out").child(currentDateTimeString).child("Room");;
                DatabaseReference reference1 = mFirebaseDatabase1.child(RoomName).child(date).child("Sign-out").child(currentDateTimeString).child("User");
                reference1.setValue(userEmail);
                String resultnew = result.toString();
                reference.setValue(resultnew);

                Toast.makeText(getApplicationContext(), "Ready for inspection!!",
